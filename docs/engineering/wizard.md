...
In Pi, invoke it with `/skill:wizard`. In Claude Code and harnesses where skills.sh installs bare commands, use `/wizard`. The agent can also reach for it on its own. When it hits a step you have to take, such as a key it can't mint or a dashboard it can't click, it builds you a wizard instead of writing the instructions into the chat, where they scroll away.
