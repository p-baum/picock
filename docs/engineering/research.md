...
Reach for it when the next step is *finding something out*: how an API behaves, what a spec actually says, or whether a claim holds. For sharpening a plan by interview instead of by reading, use [grilling](https://aihero.dev/skills-grilling); for exploring what to build with throwaway code, use [prototype](https://aihero.dev/skills-prototype).
