...
In Pi, invoke it with `/skill:grilling`. In Claude Code and harnesses where skills.sh installs bare commands, use `/grilling`. The agent can also reach for it automatically when a task fits — this is the underlying primitive, not a user-only entry point.
