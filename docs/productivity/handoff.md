...
In Pi, invoke it with `/skill:handoff [what the next session will be used for]`, for example `/skill:handoff continue the authentication refactor`. The focus is optional; omit it for a general handoff. In Claude Code and harnesses where skills.sh installs bare commands, use the equivalent `/handoff [focus]`. The agent won't reach for it on its own.
